...
public class MailNotification extends AbstractNotification implements
		INotification {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory
			.getLogger(MailNotification.class);
	...
	public String smtpPassword = "duino1234"; // should be put in configuration
												// file
	...
	public void execute(final TemperatureResults results) throws CoreException {
		...
		try {
			...
			email.setAuthenticator(new DefaultAuthenticator(smtpLogin,
					smtpPassword));
			// email.setDebug(true);
			...
			email.getMailSession().getProperties()
					.put("mail.smtps.port", String.valueOf(smtpPort));
			...
			String graphFilename = rrd.graph(Beerduino.getInstance()
					.getPreferences().getWorkDir()
					+ "temperatures", rrd.getRrdDb().getArchive(0)
					.getStartTime(), Util.getTime(), "Temperatures");

			TemplatedGraph graph = new TemplatedGraph();
			if (graphFilename.startsWith("/")) {
				graphFilename = "file://" + graphFilename; // cas où les
															// fichiers générés
															// ont un path
															// complet
			} else {
			...
			String cid = email.embed(new URL(graphFilename), "Temperatures"); // graphFileName
																				// est
																				// supposé
																				// avoir
																				// un
			...
			graph.setHeight(String
					.valueOf((int) (RRDTemperature.GRAPH_IMAGE_HEIGHT * 0.75)));
			graph.setWidth(String
					.valueOf((int) (RRDTemperature.GRAPH_IMAGE_WIDTH * 0.75)));
			...
			if (results.getState().compareTo(TemperatureState.OVERHEAT) == 0) {
				layout.addContext("explanation",
						"Une ou plusieurs sondes dépassent la température définie en seuil !");
			} else {
				layout.addContext("explanation",
						"Toutes les sondes sont à présent revenues sous les limites.");
			}
		 ...
		 * 
		 * // Needs javaee-api.jar MimeMessage message = new
		 * MimeMessage(session); message.setFrom(new
		 * InternetAddress(smtpLogin));
		 * message.setRecipients(Message.RecipientType.TO,
		 * InternetAddress.parse(recipient)); message.setSubject("Beerduino");
		 ...
		 * transport.close(); } catch (MessagingException e) { throw new
		 * CoreException(e); }
		 */
	}
}
