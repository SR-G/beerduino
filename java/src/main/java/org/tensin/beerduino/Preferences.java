...
public class Preferences {
	 ...
	 *             the core exception
	 */
	public static Preferences loadConfiguration(
			final String configurationFilename) throws CoreException {
		File source = new File(configurationFilename);
		if (!source.isFile()) {
			throw new CoreException("Configuration file ["
					+ configurationFilename + "] not found");
		} else {
			try {
	...
	/** The notifiers. */
	@ElementListUnion({
			@ElementList(entry = "mail", inline = true, type = MailNotification.class),
	...
	public void saveConfiguration(final String configurationFilename) {
		...
		try {
			...
			LOGGER.error("Error while writing preferences to ["
	 ...
	 */
	public void setNoNamespaceSchemaLocation(
