...
void network_loop() {
    ...
    if(plen!=0) {
        ...
        if (buf[IP_PROTO_P]==IP_PROTO_TCP_V&&buf[TCP_DST_PORT_H_P]==0&&buf[TCP_DST_PORT_L_P]==WEBSERVER_PORT) {
...
/*
// Unverified refactoring

uint16_t writeBufferToBuffer(uint8_t *ascii, uint8_t *buffer, uint16_t pos, long value) {
    int i=0;
    while (ascii[i]) {
        buffer[TCP_CHECKSUM_L_P+3+pos]=ascii[i++];
        pos++;
    }
    return pos;
}

uint16_t writeLongToBuffer(uint8_t *buffer, uint16_t pos, long value) {
    char ascii[50];// needs to be this big to hold a type float
    ltoa(value, ascii, 10);
    return writeBufferToBuffer(ascii, buffer, pos, value);
}

uint16_t writeIntToBuffer(uint8_t *buffer, uint16_t pos, int value) {
    char ascii[50];// needs to be this big to hold a type float
    ltoa(value, ascii, 10);
    return writeBufferToBuffer(ascii, buffer, pos, value);
}

uint16_t writeFloatToBuffer(uint8_t *buffer, uint16_t pos, float value) {
    char ascii[20];// needs to be this big to hold a type float
    dtostrf(value,10,2,ascii);
    return writeBufferToBuffer(ascii, buffer, pos, value);
}
*/
