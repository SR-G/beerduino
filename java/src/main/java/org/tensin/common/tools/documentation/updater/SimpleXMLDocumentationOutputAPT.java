...
public class SimpleXMLDocumentationOutputAPT implements
	...
	protected String format(final SimpleXMLDocumentationEntity currentEntity,
		...
		if (currentEntity.getType().isEnum()) {
			...
			// sbEnum.append("*-----------------+\n");
			List<String> enumerations = currentEntity.getType()
					.getEnumeration();
		...
		/* Attributs */
		int initialAttributeLineSize = ("&lt;" + currentEntity.getType()
		...
		while (itEntities.hasNext()) {
			...
			SimpleXMLDocumentationEntity entity = currentEntity.getEntities()
					.get(name);
		...
		if (currentEntity.getType().getName().indexOf(".") > -1) {
			...
			String packageName = fullName.substring(0,
		...
		while (itEntities.hasNext()) {
			String name = itEntities.next();
			SimpleXMLDocumentationEntity subEntity = currentEntity
	...
	private String summaryInAPT(final int profondeur,
		...
		Collection<SimpleXMLDocumentationEntity> subEntities = root
				.getEntities().values();
